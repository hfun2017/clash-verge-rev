...
pub static APP_ID: &str = "io.github.hfun2017.nethelper";
#[cfg(not(feature = "verge-dev"))]
pub static BACKUP_DIR: &str = "nethelper-backup";

#[cfg(feature = "verge-dev")]
pub static APP_ID: &str = "io.github.hfun2017.nethelper.dev";
#[cfg(feature = "verge-dev")]
pub static BACKUP_DIR: &str = "nethelper-backup-dev";
...
pub fn service_path() -> Result<PathBuf> {
    ...
    Ok(res_dir.join("nethelper-service"))
}

#[cfg(windows)]
pub fn service_path() -> Result<PathBuf> {
    let res_dir = app_resources_dir()?;
    Ok(res_dir.join("nethelper-service.exe"))
}
...
pub fn ipc_path() -> Result<PathBuf> {
    ensure_mihomo_safe_dir()
        .map(|base_dir| base_dir.join("nethelper").join("nethelper-mihomo.sock"))
        .or_else(|| {
            app_home_dir()
                .ok()
                .map(|dir| dir.join("nethelper").join("nethelper-mihomo.sock"))
        })
