...
impl IVerge {
    /// 有效的clash核心名称 (renamed: verge-mihomo -> nethelper-mihomo to match tauri.conf.json externalBin)
    pub const VALID_CLASH_CORES: &'static [&'static str] = &["nethelper-mihomo", "nethelper-mihomo-alpha"];
        ...
        if let Some(ref core) = config.clash_core {
            ...
            if core_str.is_empty() || !Self::VALID_CLASH_CORES.contains(&core_str) {
                logging!(
                    warn,
                    Type::Config,
                    "启动时发现无效的clash_core配置: '{}', 将自动修正为 'nethelper-mihomo'",
                    core
                );
                config.clash_core = Some("nethelper-mihomo".into());
                needs_fix = true;
            }
        } else {
            logging!(
                info,
                Type::Config,
                "启动时发现未配置clash_core, 将设置为默认值 'nethelper-mihomo'"
            );
            config.clash_core = Some("nethelper-mihomo".into());
            needs_fix = true;
        }
    ...
    pub fn get_valid_clash_core(&self) -> String {
        self.clash_core.clone().unwrap_or_else(|| "nethelper-mihomo".into())
    }
    ...
    pub fn template() -> Self {
        Self {
            ...
            clash_core: Some("nethelper-mihomo".into()),
